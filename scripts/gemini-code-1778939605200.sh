#!/usr/bin/env bash
# =====================================================================
#  VPN Prober — one-shot installer for Arch Linux
# =====================================================================
#  Usage:
...
. /etc/os-release
if [[ "${ID:-}" != "arch" ]]; then
  warn "This version is optimized for Arch Linux (you have ${PRETTY_NAME:-unknown})."
...
# ---------- 1. pacman + Base Packages ----------
log "[1/5] pacman system update + base packages"
$SUDO pacman -Syu --noconfirm
$SUDO pacman -S --needed --noconfirm ca-certificates curl gnupg openssl ufw

# ---------- 2. Docker Installation ----------
if ! command -v docker >/dev/null 2>&1; then
  log "[2/5] installing Docker and Docker Compose via pacman"
  ...
  # На всякий случай проверяем, запущен ли демон
  $SUDO systemctl enable --now docker.service
fi
...
# В Arch Linux служба UFW по умолчанию выключена, включаем её
...
══════════════════════════════════════════════════════════════════
  ✓ COORDINATOR INSTALLED (Arch Linux)
══════════════════════════════════════════════════════════════════
  Public IP: $IP
  Public URL: $DASH_URL
